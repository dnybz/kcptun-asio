...
void Session::run() {
    ...
    timer_ = std::make_shared<asio::steady_timer>(service_);
    //run_timer(boost::posix_time::microsec_clock::universal_time() + boost::posix_time::millisec(Interval));
	std::chrono::steady_clock::time_point t = std::chrono::steady_clock::now() + std::chrono::milliseconds(Interval);
	run_timer(t);
    // run_peeksize_checker();
}	

void Session::run_timer(std::chrono::steady_clock::time_point pt) {
...
void Session::update() {
    DeferCaller defer([this]{
		...
		std::chrono::steady_clock::time_point t = std::chrono::steady_clock::now() + std::chrono::milliseconds(next);
		run_timer(t);
    });
